...
module.exports = {
	name: "hdj",
	description: "blast annoying sounds hardly to everyone on the realm.",
	options: [
		{
			type: 3,
			name: "code",
		...
		},
        {
			type: 4,
			name: "amount",
			description: "the amount of sound packets to send",
			min_value: 100,
			max_value: 750
		},
		{
	...
	execute: async (interaction, args, dbUser) => {
		...
		const embed = {
			title: "Hard Realm DJ",
			description: `${reply} ${loading} Connecting to realm\n${end} ${none} Playing the DJ hard`,
			timestamp: new Date(),
			color: 65280,
			footer: {
				text: `/hdj | Command used by ${user.username}`,
