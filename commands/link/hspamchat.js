...
module.exports = {
	name: "hspamchat",
	description: "hard flood the chat of a p2w realm with a custom message.",
	options: [
		{
			type: 3,
			name: "code",
	...
	execute: async (interaction, args, dbUser) => {
		...
		let { capture_chat_log, dupe, amount } = args;
		const source = args.source ?? 2;

        if(typeof amount === "undefined") amount = 750;
		...
		//if (source >= 3 && args.message) alerts = `:warning: You chose to have a custom message to spam, but you selected the spam source to something that does not support it. :warning:\n`;

		const embed = {
			title: "Hard Chat Spam",
			description: `${reply} ${loading} Connecting to realm\n${end} ${none} Sending hard chat packets`,
			timestamp: new Date(),
			color: 65280,
			footer: {
				text: `/hspamchat | Command used by ${user.username}`,
		...
		client.on("play_status", (packetData) => {
			...
			setTimeout(() => {
				...
				embed.description = `${reply} ${check} Connecting to realm\n${reply} ${check} Sending chat packets\n${reply} Operation Completed.\n${end} with an average of ${(messageCount / time).toFixed(2)} messages was sent per second.`;
